...
do
    ...
    # 运行容器，并设置重启策略为always
    container_id=$(docker run -d --restart always -v "$storage_path:/root/.titanedge/storage" --name "titan$i" nezha123/titan-edge:1.3 --url https://hk-locator.titannet.io:5000/rpc/v0)
